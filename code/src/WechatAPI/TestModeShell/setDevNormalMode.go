...
/*
@cmt 20180915
@描述： 用于从 normal_mode_config.txt中 **批量** 导入配置 并 发送 *设置正常模式*命令 给相应节点设备
@compile: go build setDevNormalMode.go
@用法步骤:  1.部署了DeviceServer, WechatAPI
           2.配置好了  normal_mode_condig.txt
           3.运行 ./setDevNormalMode [gw_name]   (e.g. ./setDevNormalMode 1AAA01000151)
@format of normal_mode_config.txt:
          [device_mac]
...
func main() {
    ...
    for {
        ...
        httpServerIP := fmt.Sprintf("http://172.18.247.53:8990/set-normal-mode?gwid=%s&deviceid=%s&requestid=1234567890123",
                                    gatewayID, devicemac )
